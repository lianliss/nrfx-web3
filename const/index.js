module.exports = {
  localhost: 'http://localhost',
  REFER_NRFX_ACCRUAL: 0.05,
  TOKENS: [
    'nrfx',
    'usdt',
    'busd',
    'bnb',
    'ton',
  ],
  FIATS: [
    'rub',
    'idr',
    'uah',
    'cny',
    'cad',
    'vnd',
    'thb',
    'pln',
    'try',
    'eur',
    'gbp',
  ],
  FIAT_ADDRESS: {
    USD: '0xc0Bd103de432a939F93E1E2f8Bf1e5C795774F90',
    EUR: '0xa702e05965FEd09FDDFE4ca182b0915CdBa367c8',
    RUB: '0xC7b9dA3D064a918B8e04B23AEEdBD64CBa21D37d',
    UAH: '0xcAA5eb94f5339a598580A68f88F1471c36599dDA',
    CNY: '0xA61Feb03EB111373a84A4b303Ea391140fa3291c',
    IDR: '0x814b62d5a157498145c59820763430Ce7558bA6e',
    PLN: '0x815fe8056d867052bde314018166f144c11f6c4c',
    THB: '0xf21311db1d6ae2538dc86a0bbc751c53439e0895',
    VND: '0x9a630ef70abf193bb24b082d7a10c515c0e847c6',
    CAD: '0x1ade4f9b177a42b160cb304ce402f1daabfb2d2d',
    TRY: '0x8845161A0EA235F9e94c815241A0e63AcbaC144B',
    GBP: '0xC00565016486b345BefdD38c6BEA3A4E497F7633',
  },
  FIATS_PAYTYPES: {
    RUB: ['TinkoffNew'],
    IDR: ['BANK'],
    UAH: ['PrivatBank', 'Monobank'],
    CNY: ['BANK'],
    USD: ['BANK'],
    CAD: ['InteracETransfer'],
    VND: ['BANK'],
    THB: ['BANK'],
    TRY: ['Ziraat', 'VakifBank'],
    EUR: ['SEPAinstant'],
    GBP: ['Wise'],
    PLN: ['Blik', 'PKObank', 'Millennium'],
  },
  USDT_ADDRESS: '0x55d398326f99059ff775485246999027b3197955',
  GET_RATE_INTERVAL: 60 * 1000,
  GET_BINANCE_RATE_INTERVAL: 60 * 2 * 1000,
  GET_BINANCE_WITHDRAWS_INTERVAL: 1000 * 30,
  ZERO_ADDRESS: '0x0000000000000000000000000000000000000000',
  FIAT_FACTORY: '0xF9ceb479201054d2B301f9052A5fFBe47D652358',
  EXCHANGE_ROUTER: '0x94d0376ba6617D96c8c89213de81F4307e4490d0',
  EXCHANGE_POOL: '0x40fA05f47C0aC1033Fa94e4d6aD398B4BB4d1007',
  ORACLE_CONTRACT: '0xE948F3AE41105118A48B0a656f59C5B4113d404e',
  DEFAULT_REFER: '0x08AbC7831db337419579EC1CD36460B47A1Df492',
  DEFAULT_REFERRAL_PERCENT: 0.6,
};